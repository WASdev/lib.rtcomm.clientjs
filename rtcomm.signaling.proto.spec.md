...
|  Key                   | Value                                     |
| ----------------------|:-------------------------------------------|
|   type              |  "offer", "answer", "pranswer", "icecandidate"|
|   sdp|candidate     | Raw data (based on standards for RTCPeerConnection| 
...
An endpoint declares this information by publishing a 'DOCUMENT' message.  This message is an MQTT Retained message that is consumed by the service which stores this as registration information. The service stores this registration until the endpoint goes away (either purposefully or accidentally).  When it goes away it sends an empty message to the topic it published this DOCUMENT.  The service then removes the registration.
