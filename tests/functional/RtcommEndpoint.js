...
registerSuite({
  ...
  "Endpoint creation(iceServers)": function() {
    ...
    Fat.createProvider(getConfig(), 'test').then(
      function(ep) {
        endpoint = ep.createRtcommEndpoint({
          webrtc: true,
          chat: true
        });
        // mark for destroy.
  ...
  "in Browser A calls B": function() {
    ...
    createAndInitTwoProviders(cfg1, cfg2)
      .then(function(obj) {
        ...
        ep1 = EP1.getRtcommEndpoint({
          webrtc: false,
          chat: true
        });
        ep2 = EP2.getRtcommEndpoint({
          webrtc: false,
          chat: true
        });
  ...
  "in Browser A calls B(disconnect while ringing)": function() {
    ...
    createAndInitTwoProviders(cfg1, cfg2)
      .then(function(obj) {
        ...
        ep1.on('session:failed', function() {
          ep1_failed = true
        });
        ep2.on('session:failed', function() {
          ep2_failed = true
        });
        ep1.on('session:trying', function() {
          ep1_trying = true
        ...
        ep2.on('session:started', function() {
          ep2_started = true
        });
        ep1.on('session:stopped', function() {
          console.log('EP1 ***session:stopped');
          ep1_stopped = true;
        });
  ...
  "Customer A calls Queue[Toys], establish session": function() {
    ...
    var customer = endpointProvider.createRtcommEndpoint({
      webrtc: false,
      chat: true
    });
    var agent = endpointProvider2.createRtcommEndpoint({
      webrtc: false,
      chat: true
    });
